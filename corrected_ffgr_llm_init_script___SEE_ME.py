...
{{
  ...
  "reasoning": "brief explanation of capacity management strategy"
    ...
    def _get_semantic_guidance(self, problem) -> dict:
        """LLM guidance for semantic knapsack problems loaded from JSON."""

        # Summarize categories and relationship structure
        category_summary = {}
        total_weight = 0
        total_items = 0
        ...
        for item in problem.items:
            ...
            # Relationship stats (semantic JSON)
            requires_total += len(getattr(item, 'requires', []) or [])
            conflicts_total += len(getattr(item, 'conflicts', []) or [])
            synergies_total += len(getattr(item, 'synergies', []) or [])
        ...
        avg_item_weight = total_weight / max(total_items, 1)
        safe_prob = (problem.capacity * 0.85) / (len(problem.items) * max(avg_item_weight, 1e-9))

        semantic_stats = {
            'items': len(problem.items),
            'avg_item_weight': avg_item_weight,
            'avg_requires_per_item': requires_total / max(total_items, 1),
            'avg_conflicts_per_item': conflicts_total / max(total_items, 1),
            'avg_synergies_per_item': synergies_total / max(total_items, 1),
            'avg_synergy_bonus': synergy_bonus_total / max(total_items, 1),
            'avg_conflict_penalty': conflict_penalty_total / max(total_items, 1),
        }
...
SEMANTIC STRUCTURE (averages):
{json.dumps(semantic_stats, indent=2)}

CATEGORY SUMMARY:
{json.dumps(category_summary, indent=2)}

CONSTRAINT / RELATIONSHIP PENALTIES (avoid these!):
- Over capacity: 10 points per unit over
- Missing categories: 100 points per missing category
- Missing a required item: 200 points EACH unmet requirement
- Conflicts: add the item's conflict_penalty when conflicting items are both selected
- Synergies: add the item's synergy_bonus when synergistic items are both selected
...
TASK:
Provide an initialization strategy that (1) respects capacity, (2) helps satisfy min-categories,
and (3) is conservative about dependency/conflict risks.

Respond in JSON:
{{
  "selection_probability": 0.XX (MUST be ≤ {safe_prob:.3f}),
  "category_priorities": {{"category_name": X.X, ...}} (values 0.8-1.5),
  "min_value_weight_ratio": X.X,
  "reasoning": "brief explanation focusing on capacity + dependency/conflict risk"
}}"""
        ...
        try:
            ...
            # Safety clamp on selection probability
            if guidance.get('selection_probability', 0.0) > safe_prob:
                guidance['selection_probability'] = safe_prob * 0.9
            ...
            return {
                "selection_probability": min(0.10, safe_prob * 0.9),
                "category_priorities": {cat: 1.0 for cat in category_summary.keys()},
...
PROMPT_VERSION = "v1.1-semantic-cachek-2026-01-25"
...
class GeneticAlgorithm:
    ...
    def initialize_population_llm_guided(self, guidance: dict):
        """FIXED: Constraint-aware LLM-guided initialization"""
        population = []
        
        # Generate diverse solutions using greedy + randomization
        for pop_idx in range(self.population_size):
            # Start empty
            solution = [False] * len(self.problem.items)
            
            # Score all items according to LLM guidance
            item_scores = []
            for i, item in enumerate(self.problem.items):
                # Base score from LLM guidance
                base_score = guidance.get('selection_probability', 0.1)
                
                # Category priority
                cat_mult = guidance.get('category_priorities', {}).get(
                    item.category if hasattr(item, 'category') else 'default', 1.0
                )
                
                # Priority multiplier (for hard problems)
                if hasattr(item, 'priority_level'):
                    priority_mult = guidance.get('priority_multipliers', {}).get(
                        str(item.priority_level), 1.0
                    )
                else:
                    priority_mult = 1.0
                
                # Value/weight ratio bonus
                ratio = item.value / max(item.weight, 1)
                ratio_threshold = guidance.get('min_value_weight_ratio', 2.0)
                ratio_mult = 1.3 if ratio > ratio_threshold else 1.0
                
                # Combined score (using value/weight as base metric)
                score = ratio * base_score * cat_mult * priority_mult * ratio_mult
            ...
            # FIXED: Greedy selection with constraint checking
            current_weight = 0
            categories = set()
            category_counts = {}
            priority_1_count = 0
            
            # Track what we need to satisfy constraints
            ...
            # First pass: Add items greedily while respecting capacity
            for item_idx, score, item in item_scores:
                # Check if we can add this item
                would_exceed_capacity = (current_weight + item.weight > self.problem.capacity)
                
                # Check category limit
                cat = item.category if hasattr(item, 'category') else 'default'
                current_cat_count = category_counts.get(cat, 0)
                would_exceed_cat_limit = (current_cat_count >= max_per_category)
                
                # Add if feasible
                if not would_exceed_capacity and not would_exceed_cat_limit:
                    solution[item_idx] = True
                    current_weight += item.weight
                    categories.add(cat)
                    category_counts[cat] = current_cat_count + 1
                    
                    if hasattr(item, 'priority_level') and item.priority_level == 1:
                        priority_1_count += 1
            
            # Second pass: Ensure minimum constraints are met
            # If we don't have enough categories or priority items, add more (still respecting constraints).
            if len(categories) < min_categories or priority_1_count < min_priority_1:
                for item_idx, score, item in item_scores:
                    if solution[item_idx]:  # Already selected
                        continue
                    
                    cat = item.category if hasattr(item, 'category') else 'default'
                    need_category = (cat not in categories and len(categories) < min_categories)
                    need_priority = (hasattr(item, 'priority_level') and
                                     item.priority_level == 1 and
                                     priority_1_count < min_priority_1)
                    
                    if not (need_category or need_priority):
                        continue
                    
                    # Respect capacity + max-per-category
                    if current_weight + item.weight > self.problem.capacity:
                        continue
                    if category_counts.get(cat, 0) >= max_per_category:
                        continue
                    
                    # Add the item and update bookkeeping
                    solution[item_idx] = True
                    current_weight += item.weight
                    category_counts[cat] = category_counts.get(cat, 0) + 1
                    categories.add(cat)
            ...
            # Third pass: Add randomness for diversity (10-20% of slots)
            # This prevents all solutions from being identical, while maintaining bookkeeping correctness.
            diversity_factor = 0.1 + (pop_idx / self.population_size) * 0.1
            ...
            for _ in range(n_swaps):
                ...
                if solution[idx]:
                    ...
                    would_lose_priority = (hasattr(item, 'priority_level') and
                                          item.priority_level == 1 and
                                          priority_1_count <= min_priority_1)
                    ...
                    # Try to add
                    if current_weight + item.weight <= self.problem.capacity and category_counts.get(cat, 0) < max_per_category:
                        solution[idx] = True
                        current_weight += item.weight
                        category_counts[cat] = category_counts.get(cat, 0) + 1
                        categories.add(cat)
